...
f=open('data//depth_round2.txt','w')
for depth in range(3,15):
    for num_round in [x for x in range(50,103,4)]:
        print('loading data...')
        ...
        print('max_depth='+str(depth)+',num_boost_round='+str(num_round))
        ...
        for i,(train_peptide,test_piptide) in enumerate(cv):
            ...
            tmp = time.time()
            bst = xgb.train(plst,xgtrain,num_boost_round=num_round)
        ...
        f.write(str(depth)+'\t'+str(num_round)+'\t'+str(person_mean)+'\n')
f.close()
